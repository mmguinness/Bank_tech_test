...
describe Account do
  ...
  describe '#credit' do
    ...
    it 'records the transaction with details of amount credited, todays date and new balance' do
      account.credit(1000)
      expect(account.history[0]).to eq({ :date => "28/03/2022", :credit => 1000, :debit => 0, :balance => 1000 })
    end

    it 'records the transaction with date provided, if required' do
      account.credit(1000, "30/03/2022")
      expect(account.history[0]).to eq({ :date => "30/03/2022", :credit => 1000, :debit => 0, :balance => 1000 })
    end
  ...
  describe '#debit' do
    ...
    it 'records the transaction with details of amount debited, date and new balance' do
      account.credit(1000)
      account.debit(500)
      expect(account.history[1]).to eq({ :date => "28/03/2022", :credit => 0, :debit => 500, :balance => 500 })
    end
  end
